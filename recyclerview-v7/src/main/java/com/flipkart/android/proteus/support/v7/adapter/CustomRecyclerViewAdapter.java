...
public class CustomRecyclerViewAdapter extends ProteusRecyclerViewAdapter<ProteusViewHolder> {
    ...
    private static final String KEY_CHECKBOX_ID = "checkboxId";
    ...
    public void onBindViewHolder(ProteusViewHolder holder, int position) {
        ...
        if (itemObjectValue != null) {
            String checkBoxId = itemObjectValue.getAsString(KEY_CHECKBOX_ID);
                    ...
                    checkBox.setOnCheckedChangeListener((buttonView, isChecked) -> {
                        if (!onBind) {
                            ...
                            for (int i = 0; i < count; i++) {
                                ...
                                if (item != null) {
                                    int overDueAmount = item.getAsInteger(item.getAsString(KEY_OVER_DUE_AMOUNT), 0);
