...
import com.flipkart.android.proteus.support.R;
import com.flipkart.android.proteus.support.callback.OnOtpCompletionListener;

/**
 * Created by Prasad Rao on 30-04-2020 12:10
 **/
public class OtpEntryEditText extends AppCompatEditText {
    ...
    private static final int[] FILLED_STATE = new int[]{R.attr.state_filled};
    ...
    public OtpEntryEditText(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.otpViewStyle);
    }

    public OtpEntryEditText(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray =
            theme.obtainStyledAttributes(attrs, R.styleable.OtpEntryEditText, defStyleAttr, 0);
        viewType = typedArray.getInt(R.styleable.OtpEntryEditText_viewType, VIEW_TYPE_NONE);
        otpViewItemCount = typedArray.getInt(R.styleable.OtpEntryEditText_itemCount, DEFAULT_COUNT);
        otpViewItemHeight =
            (int) typedArray.getDimension(R.styleable.OtpEntryEditText_itemHeight,
                res.getDimensionPixelSize(R.dimen.otp_view_item_size));
        otpViewItemWidth =
            (int) typedArray.getDimension(R.styleable.OtpEntryEditText_itemWidth,
                res.getDimensionPixelSize(R.dimen.otp_view_item_size));
        otpViewItemSpacing =
            typedArray.getDimensionPixelSize(R.styleable.OtpEntryEditText_itemSpacing,
                res.getDimensionPixelSize(R.dimen.otp_view_item_spacing));
        otpViewItemRadius =
            (int) typedArray.getDimension(R.styleable.OtpEntryEditText_itemRadius, 0);
        lineWidth =
            (int) typedArray.getDimension(R.styleable.OtpEntryEditText_lineWidth,
                res.getDimensionPixelSize(R.dimen.otp_view_item_line_width));
        lineColor = typedArray.getColorStateList(R.styleable.OtpEntryEditText_lineColor);
        isCursorVisible =
            typedArray.getBoolean(R.styleable.OtpEntryEditText_android_cursorVisible, true);
        cursorColor =
            typedArray.getColor(R.styleable.OtpEntryEditText_cursorColor, getCurrentTextColor());
        cursorWidth =
            typedArray.getDimensionPixelSize(R.styleable.OtpEntryEditText_cursorWidth,
                res.getDimensionPixelSize(R.dimen.otp_view_cursor_width));
        itemBackground =
            typedArray.getDrawable(R.styleable.OtpEntryEditText_android_itemBackground);
        hideLineWhenFilled =
            typedArray.getBoolean(R.styleable.OtpEntryEditText_hideLineWhenFilled, false);
        rtlTextDirection =
            typedArray.getBoolean(R.styleable.OtpEntryEditText_rtlTextDirection, false);
        maskingChar = typedArray.getString(R.styleable.OtpEntryEditText_maskingChar);
        typedArray.recycle();
        if (lineColor != null) {
    ...
    private void setMaxLength(int maxLength) {
        setFilters(maxLength >= 0 ? new InputFilter[]{
            new InputFilter.LengthFilter(maxLength)} : NO_FILTERS);
    }
        ...
        for (int i = 0; i < otpViewItemCount; i++) {
            ...
            if (rtlTextDirection) {
                ...
                if (getText().length() >= reversedPosition) {
                    ...
                    getHint().length() == otpViewItemCount) {
                    drawHint(canvas, i);
                }
            } else {
                if (getText().length() > i) {
    ...
    private void drawInput(Canvas canvas, int i) {
        ...
        if (maskingChar != null &&
            (isNumberInputType(getInputType()) || isPasswordInputType(getInputType()))) {
    ...
    private int getLineColorForState(int... states) {
        return lineColor !=
            null ? lineColor.getColorForState(states, cursorLineColor) : cursorLineColor;
    }
    ...
    private void drawOtpLine(Canvas canvas, int i) {
        ...
        updateRoundRectPath(itemLineRect, otpViewItemRadius, otpViewItemRadius, drawLeft,
            drawRight);
        canvas.drawPath(path, paint);
    }
    ...
    private void updateRoundRectPath(RectF rectF, float rx, float ry, boolean tl, boolean tr,
    ...
    private void updateItemRectF(int i) {
        float halfLineWidth = ((float) lineWidth) / 2;
        float left = getScrollX() + ViewCompat.getPaddingStart(this) +
    ...
    private void drawMaskingText(Canvas canvas, int i, String maskingChar) {
        ...
        if (rtlTextDirection) {
            ...
            if (reversedCharPosition <= 0 && getText() != null) {
                drawTextAtBox(canvas, paint, getText().toString().replaceAll(".", maskingChar), Math
    ...
    private void updateColors() {
        ...
        int color = lineColor !=
            null ? lineColor.getColorForState(getDrawableState(), 0) : getCurrentTextColor();
     ...
     * @attr ref R.styleable#OtpEntryEditText_lineColor
     ...
     * Sets the line width.
     *
     * @attr ref R.styleable#OtpEntryEditText_lineWidth
     * @see #getLineWidth()
     */
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_itemCount
     * @see #getItemCount()
     */
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_itemRadius
     * @see #getItemRadius()
     */
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_itemSpacing
     * @see #getItemSpacing()
     */
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_itemHeight
     * @see #getItemHeight()
     */
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_itemWidth
     * @see #getItemWidth()
     */
     ...
     * @attr ref R.styleable#OtpEntryEditText_hideLineWhenFilled
     */
     ...
     * @attr ref R.styleable#OtpEntryEditText_android_itemBackground
     */
    public void setItemBackgroundResources(@DrawableRes int resId) {
        ...
        itemBackground =
            ResourcesCompat.getDrawable(getResources(), resId, getContext().getTheme());
     ...
     *
     * @attr ref R.styleable#OtpEntryEditText_cursorWidth
     * @see #getCursorWidth()
     */
     ...
     * Do not pass a resource ID. To get a color value from a resource ID, call
     * {@link androidx.core.content.ContextCompat#getColor(Context, int) getColor}.
     * @attr ref R.styleable#OtpEntryEditText_cursorColor
     * @see #getCursorColor()
     */
    ...
    public void setViewType(String type) {
        if ("line".equalsIgnoreCase(type)) {
            this.viewType = VIEW_TYPE_LINE;
        } else if ("rectangle".equalsIgnoreCase(type)) {
            this.viewType = VIEW_TYPE_RECTANGLE;
        } else {
            this.viewType = VIEW_TYPE_NONE;
        }
        invalidate();
    }
